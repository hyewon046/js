...
// 객체는 이터러블이 아니지만 스프레드 문법 사용 가능
console.log({...{x:1, y:2}}); //{x:1, y:2}
// 객체 스프레드시 프라퍼티가 중복되면 마지막 프라퍼티만 남는다
console.log({...{x:1, y:2}, ...{y:3, z:4}}); //{x:1, y:3, z:4}, 뒤에 y,z땜에 앞에 y:2가 덮어짐
...
// 구조분해할당시 기본값 설정
const [aa, bb=2, cc] = [1,,3]; //bb는 값이 있으므로 값을 안넣어도됨
...
console.log(a1, b1, c1, d1, e1); //H i undefined undefined undefined

// 문자열 객체의 length프라퍼티 추출
const str = 'Hello';
const {length} = str;
console.log(length);
...
console.log(nvar, avar); //undefined undefined
