...
class Person {
    ...
    constructor(name, age) {
        // 프라퍼티명과 파라미터명을 다르게 해야함
        this._name = name; //순환참조 방지(_)
        this._age = age; //순환참조 방지(_)
    }
    // getter, 프로토타입 메소드
    get name() {
        return this._name; //순환참조 방지(_)
    }
    // setter, 프로토타입 메소드
    set name(name) {
        this._name = name; //순환참조 방지(_)
...
// 프라퍼티 접근
// 프라퍼티명과 생성자함수 파라미터명이 같으면(this.name 으로 적을시) RangeError
// : name에 접근 > getter 호출 > name에 접근 ... (순환참조)
console.log(person1.name); //getter 호출
console.log(person2.name); //getter 호출

// 프라퍼티 변경
// 프라퍼티명과 생성자함수 파라미터명이 같으면(this.name(age)로 적을시) RangeError
...
//person1.printName('최영'); //클래스명으로 접근하지 않을시 Error