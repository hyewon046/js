...
console.log('노드 값 : ', mainTitle.nodeValue); //노드값이 없음
...
// 3. 문서노드인 경우 (document앞에 window. 생략됨)
console.log('노드 타입 : ', document.nodeType);
console.log('노드 이름 : ', document.nodeName);
console.log('노드 값 : ', document.nodeValue); //노드값이 없음
...
mainTitle.textContent = '수정된 textContents';
console.log('textContent : ', mainTitle.textContent);

console.log('innerHTML : ', list.innerHTML);
list.innerHTML += "<li data-id='4' data-name='Item4'>Itme 4</li>";
console.log('innerHTML : ', list.innerHTML);

// 요소의 속성들
console.log('attributes : ', mainTitle.attributes);
//NNamedNodeMap {0: id, 1: class, id: id, class: class, length: 2}
...
// DocumentFragment : 독림된 문서객체(DOM 트리를 벗어난 새로운 DOM), 직독직해:문서조각
...
console.log('computec style : ', getComputedStyle(mainTitle).color);
console.log('computec style : ', getComputedStyle(mainTitle));