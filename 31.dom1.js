...
document.getElementById('testButton').addEventListener('click', () => {
    ...
    console.log("아이디가 parent인 요소의 자손 요소 중 클래스가 child인 요소 하나 : ",
        querySingle);

    const queryAll = document.querySelectorAll('#parent .child');
    console.log("아이디가 parent인 요소의 자손 요소 중 클래스가 child인 모든 요소 : ",
        queryAll);

    // 실습 1. id가 parent인 요소의 세번째 자식 요소
    const querythree = document.querySelector('#parent .child').nextSibling;
    ...
    // const childsecond = document.getElementsByClassName('child');
    // console.log(childsecond); //실패작
